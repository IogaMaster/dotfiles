...
{ lib }:
let inherit (lib) attrByPath hasAttrByPath optional optionalAttrs;
in rec {
  ...
  extractCPU = facter:
    ...
    in optionalAttrs (cpu != null) {
      ...
      cores = (lib.strings.match ".*([0-9]+)-core.*" (cpu.version or ""))
        != null;
    };

  # Extract GPU/Display information from facter data
  extractGPU = facter:
    let devices = attrByPath [ "hardware" "display" ] [ ] facter;
    in if lib.length devices > 0 then {
      hasDisplay = true;
  ...
  extractNetwork = facter:
    let devices = attrByPath [ "hardware" "network" ] [ ] facter;
    in {
  ...
  isASUSROG = facter:
    ...
    in (lib.hasPrefix "ASUS" manufacturer) || (lib.hasInfix "ROG" product);

  # Detect if system has battery (laptop)
  isLaptop = facter:
    let powerSupply = attrByPath [ "hardware" "power_supply" ] [ ] facter;
    in lib.any (ps: ps.type == "Battery") powerSupply;
  ...
  detectHybridGPU = facter:
    ...
    in intelGPU && nvidiaGPU;

  # Complete hardware summary extraction
  summarizeHardware = facter:
    let system = attrByPath [ "hardware" "system" ] { } facter;
    in {
      systemInfo = {
